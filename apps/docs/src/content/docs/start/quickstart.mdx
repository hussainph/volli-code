...
organizes tasks. Board chats and files open as tabs beside it.

![The Home Board for voltaic with its five columns and 24 tickets fit on screen; the navigation sidebar is hidden, while the Chat control remains in the top bar.](../../../assets/screenshots/board.png)
...
1. **On Home, select Chat.**
   ...
   This starts a *Board chat* in your main checkout.

2. **Describe the goal in broad terms.**

   Use a Board chat when you want help shaping the work:
...
3. **Return to Board.**
   ...
   A Board chat can create and edit tickets. Review the tasks it created and
   adjust them before you start work.

</Steps>

A Board chat sees your main checkout and the whole board. Use it when the
...
## Optional: Reuse a workflow

After you have run a task normally, you can save a repeated start from
**Automations**. An Automation keeps its Instructions, Trigger, and Runtime.
Run it by hand first; later, enable it on this Mac and arm a column or add a
schedule when you want it to start automatically. See the [Automations
guide](/guides/automations/) for the full workflow.

This step is optional. Moving a ticket to **Doing** and selecting **Chat**
remain the simplest way to start your first task.
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Save a repeatable start after your first task."
  />
  <LinkCard
