...
Where an ID is optional, `volli` infers it from where you are. Inside a
ticket's worktree, it uses that ticket. Inside a session Volli started, it
knows the session too, so an agent can run `volli session done` with no
arguments.

Use `--project <name>` to target a project other than the current one.
