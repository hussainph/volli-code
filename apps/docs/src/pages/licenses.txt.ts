 ...
 *
 * The rendering lives in `@volli/font-notices`, shared with the marketing site.
 * What belongs to THIS site is the list below: the fonts it self-hosts,
 * imported from the packages it declares, so the notice is built from the same
 * bytes the bundler is copying into `dist/`. Nothing is transcribed by hand —
 * bumping the font dependency rewrites the notice on the next build.
 *
 * A font added to `astro.config.mjs` must be added here too. If it is not,
