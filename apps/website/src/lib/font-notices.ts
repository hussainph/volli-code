...
//
// volli.app self-hosts Mona Sans: the build copies the font binaries out of
// `@fontsource-variable/mona-sans` and emits them into `dist/_astro/`, so every
// deploy REDISTRIBUTES font software. Mona Sans is OFL-1.1, and clause 2 of
// that license lets a copy be bundled with any software only "provided that
// each copy contains the above copyright notice and this license". Shipping the
// .woff2 files with no notice beside them does not meet that condition.
//
...
//
// `apps/docs/src/lib/font-notices.ts` is the same module for the docs site,
// which self-hosts the same family through Starlight's `customCss`. The two
// sites are separate Astro projects with no shared build, so this is duplicated
// deliberately rather than pulled through a workspace package that neither site
// would otherwise need. The duplication is held honest rather than trusted:
// `font-notices.test.ts` fails when the two copies' code stops matching, so
// only the prose that names each site is allowed to differ.
 ...
 *
 * Both entry points into the site (`index.astro`, `download.astro`) import
 * `@fontsource-variable/mona-sans/wght.css`, which is what pulls the binaries
 * into the build. A font added there has to be added here too — the dist check
 * fails the build when a font family reaches `dist/` without a notice.
 */
