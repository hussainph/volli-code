...
The libvips entry has a separate technical record:
[`docs/licensing/libvips-relink-and-resign.md`](../../../docs/licensing/libvips-relink-and-resign.md).
It records the corresponding-source inputs, the unpacked `@img` replacement
path, and the macOS re-signing/notarization path. The record is engineering
release documentation, not a legal conclusion. The existing `pnpm run
check:notices` gate remains the check for the generated notice and its packaged
resource; it does not claim to build or sign a modified app.
