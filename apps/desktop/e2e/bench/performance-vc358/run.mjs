...
// limits each repetition to the interaction this change affects.
const INTERACTIONS = Object.freeze([["new_chat", "+ Chat to usable composer"]]);
...
async function fullAppIteration({ fixtureDirectory, runRoot, armName, index, manifest }) {
  ...
  try {
    ...
    // Keep the same cold-launch/board-settle precondition as VC-353, then
    // measure only the provisional + Chat boundary.
    void cold;
...
async function runArm({ args, fixtureDirectory, runRoot, manifest, loaded, chatBuilt }) {
  ...
  try {
    ...
    void chatBuilt;
    const chat = { steps: 0, checks: {}, errors: [] };
    armResult = {
      name,
