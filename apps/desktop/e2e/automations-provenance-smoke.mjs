 ...
 *   4. The board card's ring means LIVE, not finishing: `live` is a lit ring
 ...
 *   5. Reduced motion keeps the signifier and removes only the motion: the
 *      ring is still displayed, still lit, and no longer animating.
 *
 * WHAT IT DOES NOT DRIVE, and why. The bolt itself needs a Run, a Run needs a
 * fresh Pi-backed Session, and that needs provider credentials and spends
 * tokens — so this profile has no default model and the Run below lands on the
 ...
 * and the named parent are pinned instead by the row render tests
 * (`session-band-row.test.tsx`, `ticket-sessions-panel-rows.test.tsx`) against
 * the same components this app is running, and the derivation behind them by
 * `session-provenance-repo.test.ts`.
 *
...
try {
  ...
  await attempt(4, "the rail those Sessions appear in draws no mark at all", async () => {
    ...
    const marks = await page.evaluate(() => ({
      // The bolt's own accessible name — the only resting ink this feature
      // ever adds.
      bolts: document.querySelectorAll('[aria-label^="Started by the Automation"]').length,
      // And the tooltip half, which is the whole mark for a Session another
      // Session started.
      parents: [...document.querySelectorAll("[title]")].filter((node) =>
        (node.getAttribute("title") ?? "").includes("Started by"),
      ).length,
    }));
    return {
      ok: marks.bolts === 0 && marks.parents === 0,
      detail: `bolts=${marks.bolts} parent tooltips=${marks.parents}`,
    };
  });
  ...
  await attempt(5, "a card with nothing running generates no ring at all", async () => ({
  ...
  await attempt(6, "`live` is the ring standing still, in `working`'s own colour", async () => ({
    ok:
      live.display === "block" &&
  ...
  await attempt(7, "`waiting` keeps its own reading beside the new word", async () => ({
  ...
  await attempt(8, "reduced motion removes the motion and keeps the ring", async () => ({
    ok:
  ...
  await attempt(9, "a Run with no model refuses, and marks nothing", async () => ({
