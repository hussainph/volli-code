 ...
 * simple-icons (CC0 1.0). The two the sidebar's Session marks also draw —
 * Anthropic's A and OpenAI's knot — are imported from `ui/vendor-marks.ts`
 * rather than copied, so the same vendor is the same drawing in both places
 * however differently the two surfaces colour it. The
...
import { VENDOR_MARKS } from "@renderer/components/ui/vendor-marks";
...
const FAMILY_MARK: Record<Family, Mark> = {
  ...
  openai: {
    // OpenAI's knot. Not in simple-icons — withdrawn over the vendor's
    // trademark policy — so it is a trace of the public brand SVG, held with
    // the app's other vendor path data.
    paths: [VENDOR_MARKS.openai.path],
    viewBox: "0 0 24 24",
...
const PROVIDER_MARK: Record<string, Mark> = {
  anthropic: {
    ...
    paths: [VENDOR_MARKS.anthropic.path],
    viewBox: "0 0 24 24",
