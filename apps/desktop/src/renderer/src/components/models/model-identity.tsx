 ...
 *
 * Tints follow the harness file's rule: low-chroma, brand-adjacent, off-ember.
 * Claude's own coral would sit on Volli's accent; a family colour that reads
 * as the app's own colour is worse than none.
 *
  ...
  if (mark === null) {
    ...
    return (
      <span
        aria-hidden
        className={cn(
          "inline-flex size-3.5 shrink-0 items-center justify-center rounded-sm bg-muted-foreground/20 text-label leading-none font-semibold text-muted-foreground",
          className,
        )}
      >
        {initial}
