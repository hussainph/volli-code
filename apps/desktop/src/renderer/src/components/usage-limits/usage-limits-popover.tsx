/**
 * What is left of every subscription, from anywhere in the app (VC-271).
 *
...
export function UsageLimitsPopover() {
  ...
  // Read by a resolve handler that can outlive the popover being closed.
  ...
  const load = React.useCallback(
    async (refresh: boolean) => {
      ...
      try {
        ...
        if (mounted.current) setReading({ kind: "failed" });
      } finally {
...
function ReadingView({ reading }: { reading: Reading | null }) {
  if (reading === null) {
    return (
  ...
  return (
    ...
    >
      {reading.accounts.map((account) => (
        <AccountItem key={account.providerId} account={account} />
      ))}
    </Accordion>
  );
}

function AccountItem({ account }: { account: UsageLimitAccount }) {
  return (
    <AccordionItem value={account.providerId}>
      <AccordionTrigger className="group">
        ...
        <span className="flex shrink-0 items-center gap-1.5">
          ...
          <BindingReading key={account.limits.checkedAt} window={account.binding} />
          <CaretDownIcon
            aria-hidden
      ...
      <AccordionContent>
        <AccountUsage limits={account.limits} testId={`usage-${account.providerId}`} />
      </AccordionContent>
    </AccordionItem>
  );
}
 ...
 */
function BindingReading({ window }: { window: UsageWindow | null }) {
