...
const SHIKI_LANG_IMPORTS: Readonly<Record<string, ShikiLangImporter>> = {
  ...
  // VC-125 gap fill. Ids Monaco already knows (`objective-c`, `lua`, `scala`,
  // `dart`, `elixir`, `powershell`, `r`, `clojure`, `hcl`, `proto`, `perl`,
  // `bat`) keep Monaco's bracket/comment rules under shiki's colouring; the
  // rest are registered as empty shells by `ensureMonacoLanguagesRegistered`,
  // exactly like `toml` and `cmake` above.
 ...
 * Plaintext is excluded (no grammar). Prefer lazy load via
 * {@link ensureShikiLanguage} — do not pass this list into bootstrap.
 */
 ...
 *
 * Plaintext / unknown ids are a no-op and return `false`. Already-loaded ids
 * return `true` without work.
 *
 * This only loads the grammar. Monaco TextMate providers are installed by
 * `ensureShikiLanguageBound` in `shiki-monaco.ts` after load.
 */
