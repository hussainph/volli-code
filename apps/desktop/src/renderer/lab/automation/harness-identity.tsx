 ...
 * version of the grey text it replaces. Nothing here is drawn by hand: Claude,
 * Codex and Cursor come from simple-icons (CC0 1.0), pi from its own published
 * `logo-auto.svg`, opencode from its wordmark's glyph.
 *
 * The TRADEMARKS remain the property of their owners; we use them only to
...
import { HARNESS_ADAPTERS, LAB_HARNESS_IDS, type LabHarnessId } from "./model";

export interface HarnessIdentity {
  /**
   * One or more subpaths of the vendor's own mark. A list rather than a string
   * because pi's mark is two shapes and flattening them into one `d` would be
   * editing someone else's logo.
   */
  paths: string[];
  /**
   * Kept verbatim from the source SVG rather than rescaled into 24×24 by hand —
   * retyped coordinates are how a mark quietly stops being the real one.
   */
  viewBox: string;
...
const IDENTITY: Record<LabHarnessId, HarnessIdentity> = {
  "claude-code": {
    // Claude's starburst.
    paths: [
  ...
  cursor: {
    ...
    viewBox: "0 0 24 24",
    // The cube is monochrome in every vendor rendering, so it gets a neutral —
    // a colour invented for it would be the one tint here that means nothing.
    tint: "#A8ADB6",
  },
  opencode: {
  ...
  pi: {
    // pi's own mark: a blocky p with its dot. The counter in the p is a
    // reverse-wound subpath, so the default nonzero fill rule renders the hole —
    // no `fill-rule` override needed, and none is in the source either.
    paths: [
    ...
    // Cropped to the artwork's own bounding box, not rescaled. pi publishes its
    // mark inside 800×800 with a wide margin, so at the 0-800 box every other
    // harness's glyph is 40% bigger than pi's on the same row.
    viewBox: "165 165 470 470",
    tint: "#7EA08A",
  },
};

export function harnessIdentity(harnessId: LabHarnessId): HarnessIdentity {
  return IDENTITY[harnessId];
}

export function harnessLabelFor(harnessId: LabHarnessId): string {
  return HARNESS_ADAPTERS[harnessId].label;
}
...
export function HarnessMark({
  ...
  const label = harnessLabelFor(harnessId);
  return (
    <svg
      viewBox={viewBox}
...
export function HarnessTag({
  ...
  return (
    ...
    >
      ...
      {harnessLabelFor(harnessId)}
    </span>
  );
}

/**
 * The same marks, shaped as Phosphor `Icon`s.
 *
 * The house rule is that every context-menu action carries a neighbouring icon,
 * and the shared primitive enforces it by TYPE: `icon` is an `Icon`, rendered
 * into a slot of its own. For a menu of harnesses the mark IS the icon, so the
 * choice was to adapt it or to put a meaningless glyph in the slot and the real
 * one in the label. `weight` is accepted and ignored — a brand mark has exactly
 * one weight, and pretending otherwise is the only lie this adapter tells.
 *
 * Built once at module load rather than per render: a component identity created
 * inside a render is a different component every frame, and React unmounts and
 * remounts the whole subtree each time.
 */
...
/**
 * Every harness one Automation would start, as a row of marks.
 *
 * This exists because a multi-step Automation broke an assumption the rest of
 * the app still makes: the board's drag picker, the dragged card and the index
 * row all show ONE harness, and with two steps every one of them was showing the
 * first and silently dropping the second. A trail is the smallest honest fix —
 * it occupies a mark's worth of space per harness and stops the summary lying.
 *
 * They are spaced rather than overlapped: a stack reads as "one of these" and
 * the fact being reported is "all of these".
 */
export function HarnessTrail({
  harnessIds,
  className,
}: {
  harnessIds: LabHarnessId[];
  className?: string;
}) {
  return (
