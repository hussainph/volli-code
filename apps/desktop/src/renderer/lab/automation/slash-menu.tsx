...
export function SlashMenu({
  ...
  if (skills.length === 0) {
    return (
      <div
        ...
        aria-label="Skills"
        className={cn(
          "fixed z-[80] w-72 overflow-hidden rounded-lg border border-border bg-popover py-2 shadow-overlay",
          !instant &&
  ...
  return (
    <div
      ...
      aria-label="Skills"
      className={cn(
        "fixed z-[80] w-72 overflow-hidden rounded-lg border border-border bg-popover py-1 shadow-overlay",
        !instant &&
          "transition-[opacity,transform,scale] duration-150 ease-out starting:scale-[0.97] starting:opacity-0 motion-reduce:transition-none",
      )}
