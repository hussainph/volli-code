...
describe("agent command service", () => {
  ...
  it("widens session.list with chat rows while the addressable snapshot stays terminal-only", async () => {
    ...
    // `identify` addresses that same snapshot, so a Session absent from it must
    // be unaddressable rather than half-resolved. (`session peek` reads BOTH
    // halves — VC-79 — and is exercised on its own below.)
    ...
    // `identify` still resolves terminals only: a chat exports no VOLLI_SESSION
    // of its own, so nothing can be running inside one and claiming it is.
