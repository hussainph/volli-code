...
describe("migration 020: projects.skills_auto_disclosure", () => {
  it("adds the consent column to an existing project, defaulting off", () => {
    const dbPath = tempDbPath();
    const db = openRawDb(dbPath);
    db.pragma("foreign_keys = ON");
    ...
    expect(db.pragma("user_version", { simple: true })).toBe(20);
    // Consent is never assumed for an upgraded project: disclosure stays off
    // until the user flips it in Configure.
    expect(db.prepare("SELECT skills_auto_disclosure FROM projects WHERE id = 'p1'").get()).toEqual(
      { skills_auto_disclosure: 0 },
    );
    db.close();
  });

  it("refuses anything but the honest boolean 0/1", () => {
    const dbPath = tempDbPath();
    const db = openRawDb(dbPath);
    migrate(db, dbPath);
    db.prepare(
    ...
    expect(() =>
      db.prepare("UPDATE projects SET skills_auto_disclosure = 2 WHERE id = 'p1'").run(),
    ).toThrow();
    for (const value of [0, 1]) {
      expect(() =>
        db.prepare("UPDATE projects SET skills_auto_disclosure = ? WHERE id = 'p1'").run(value),
      ).not.toThrow();
    }
    db.close();
  });
