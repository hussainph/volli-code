...
import { askToLeaveNotice, holdNoticeFor, relayHoldNotices, takeoverNotice } from "./hold-notices";
...
describe("holdNoticeFor", () => {
  it("tells the displaced Session about a takeover, in one line naming the rule its next write will cite", () => {
    const notice = holdNoticeFor({ kind: "person-took", tabId: "tab-1", displaced: A });
    expect(notice).toEqual({ sessionId: "ses-a", text: takeoverNotice("tab-1") });
  ...
  it("tells the holder it is asked to leave, and how", () => {
    const notice = holdNoticeFor({ kind: "ask-to-leave", tabId: "tab-1", holder: A });
    expect(notice).toEqual({ sessionId: "ses-a", text: askToLeaveNotice("tab-1") });
  ...
  it("owes nothing for a takeover that displaced nobody, or for the hold's own comings and goings", () => {
    const silent: BrowserHoldEvent[] = [
      { kind: "person-took", tabId: "tab-1", displaced: null },
...
describe("relayHoldNotices", () => {
  it("steers each notice into the Session it is for, and nothing for silent events", async () => {
    ...
    fake.emit({ kind: "person-took", tabId: "tab-1", displaced: A });
    fake.emit({ kind: "ask-to-leave", tabId: "tab-2", holder: A });
    await Promise.resolve();

    expect(steered).toEqual([
      { sessionId: "ses-a", text: takeoverNotice("tab-1") },
      { sessionId: "ses-a", text: askToLeaveNotice("tab-2") },
    ]);
  });
  ...
  it("logs a failed delivery and keeps going: nobody is waiting on the notice", async () => {
    ...
    expect(() => quiet.emit({ kind: "ask-to-leave", tabId: "tab-1", holder: A })).not.toThrow();
    await Promise.resolve();
  });
