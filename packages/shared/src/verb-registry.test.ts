...
/**
 * The socket surface, as VC-163 left it.
 *
 * VC-161 pinned this list to prove the registry projection reproduced the 27
 * hand-authored `AGENT_COMMANDS` strings byte for byte — that ticket re-seated
 * the surface and was forbidden from redesigning it. VC-163 is the ticket that
 * was always going to change it, and it removes exactly two:
 *
 * - `session.start` — control tier. Its `cli` access mode is gone and its actor
 *   is `role`, so the socket does not answer it for any caller. The `project`
 *   Role's `session_start` tool is the agent path; the app is the human one.
 * - `ticket.archive` — off every agent surface. App-only curation.
 *
 * Both were one registry field, and both are reversible the same way.
 */
...
describe("verbTier", () => {
  ...
  it("holds the audit's arithmetic over the socket surface", () => {
    ...
    // VC-92's 10, exactly: `ticket.archive` and `session.start` rode here until
    // VC-163, and both have now left — one off the surface, one to the tool.
    expect(socketTiers.filter((tier) => tier === "coordination")).toHaveLength(10);
