...
describe("registry ↔ argv mechanics", () => {
  ...
  // Which verbs have no argv mechanics, named rather than inferred, and now for
  // two different reasons. `hook` takes two bare positionals and never walks the
  // parser; `help` takes a command path or a topic word instead of an option
  // table. `ticket.archive` and `session.start` are the VC-163 pair: the Agent
  // CLI does not execute either, so argv mechanics for them would be a route
  // into a surface the registry no longer publishes them on.
  it("leaves exactly the verbs the CLI cannot execute without mechanics", () => {
    ...
    expect(missing).toEqual(["ticket.archive", "session.start", "hook", "help"]);
  });
});
