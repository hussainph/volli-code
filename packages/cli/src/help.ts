...
function topicText(topic: HelpTopicName, options: HelpRenderOptions): string {
  ...
  return [
    ...
    "No CLI verb waits. Nothing here blocks, and sleeping in a shell to poll is how sessions wedge — waiting is a named tool the runtime suspends the turn for, never a command. Two exist: ticket_await parks on a ticket's signals, comments and moves; session_await parks on a Session's turn ending (completed or interrupted), its done/blocked signal, or its stop. Begin Session Await with the cursor from session_start, session_send or session_delegate, then chain every wake or timeout cursor so nothing between calls is missed.",
    "Triage a fleet from session list — working, waiting (with what on), interrupted (with why), idle, or stopped, plus the age of the last durable fact — and spend session peek only where that age looks wrong. A wedged turn also self-reports: the watchdog records a blocked signal after ten silent minutes.",
