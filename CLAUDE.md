...
- Design tokens live in one canonical module: `apps/desktop/src/renderer/src/globals.css` (shadcn-style CSS variables; dark-only for now, `class="dark"` pinned in index.html). TS-consumable domain colors (project-tile palette) live in `@volli/shared`. Accent: ember orange `#E8652A`; frosted/dark two-tier sidebar aesthetic.
