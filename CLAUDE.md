...
- **Color tokens are generated, not authored.** `generateThemeTokens()` in `@volli/shared` derives the whole set from a theme's seed (`packages/shared/src/theme/`, decisions #66–#78); the renderer writes them onto `document.documentElement` at render time and main runs the same generator for the window's `backgroundColor`. `globals.css` authors the generated default (Ember) as the literal first-paint fallback — **regenerate it, never hand-tune it**, and add any new color token to `THEME_TOKEN_NAMES` or it silently keeps that fallback forever. Never persist the *resolved* token set; `{global theme, project override}` is authoritative. Non-color tokens (radius, type scale, layout) are not themeable and stay hand-authored in `globals.css`; TS-consumable domain colors (project-tile palette) live in `@volli/shared`. Dark-only, `class="dark"` pinned. Accent: ember `#E8652A` — an exact fixed point of the accent math. Spacing/width/type follow `docs/DESIGN.md` (decision #31: six-step type scale, no arbitrary `text-[Npx]`; `<ContentColumn>` for Tier-A reading surfaces, `gutter` token for Tier-B workbench edges).
