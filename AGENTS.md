...
- **Ship no third-party terminal themes.** Volli redistributes no Ghostty or iTerm2 color schemes: a terminal theme is read from the user's own machine (their Ghostty config and theme directories), and the app's own fallback is derived from its design tokens. A vendored catalog was removed in VC-413 because nobody had verified the provenance of its 463 entries; `apps/desktop/scripts/check-vendored-themes.mjs` fails CI if one returns. The same bar covers excerpts — a sixteen-colour ANSI row lifted from someone's theme is the same redistribution as the whole file.
