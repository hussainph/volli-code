...
- Route terminal access through the `TerminalEngine` interface over the preload bridge. `node-pty` never leaves `src/main`; restty never leaves renderer terminal components. `node-pty` needs `pnpm -C apps/desktop run rebuild:native` after every install to match Electron's ABI; better-sqlite3 13 is N-API and ships its own prebuild that both Electron and plain Node load, so the same command is a deliberate no-op for it.
- Keep the desktop licence notice generated, never hand-authored. `apps/desktop/THIRD-PARTY-NOTICES` is derived from the resolved production dependency set of the desktop app and the bundled `volli` CLI by `node apps/desktop/scripts/generate-third-party-notices.mjs`; `electron-builder.yml` ships it and the root `LICENSE` into the `.app` under `Contents/Resources`. What no dependency walk can see — platform-native packages, build-time sources whose output ships, vendored source, the editor-theme fragment, and notices another workspace package owns — lives as a reviewed entry in `apps/desktop/notices/sources.json`, and provenance nobody has established is recorded as unresolved rather than guessed. Regenerate after any change to the production dependency set; `pnpm -C apps/desktop run check:notices` (CI) fails on a stale notice, on packaging that stops shipping it, and on a shipped package no notice covers.
...
- Surface every failed mutation to the user with an alert or toast. Never silently swallow errors. The one exception is work no one asked for: a background refinement a person did not request (auto-titling's model call) has no failure a toast could ask them to act on, and its fallback is already on screen, so it logs and keeps the fallback. The test is whether a person is waiting on the result, not whether the code is async. Errors are for operations that failed, not for outcomes the user chose: a setting that produces a result we merely disagree with (a low-contrast canvas, a loud theme) is the user's decision, never a persistent warning or alert in Settings — if a choice deserves a caution, the control says it once at the moment of the choice and lets go. Likewise, diagnosis is our job, not the user's: a fault surface shows what is wrong and the one action that fixes it, and keeps the measurements (paths, PATH dumps, per-check evidence) in the support report rather than on the page.
...
- Give every context-menu action a neighboring Phosphor icon through the shared context-menu primitive. Outline is the baseline; use `fill` only for an exceptional item or an explicit visual requirement, `bold` for glyphs at 12px or smaller, and never `duotone`.
...
Every Volli session — ticket or project, PTY or structured — starts with the same `PATH`: Volli's bin dir first, then the login shell's exported `PATH` merged with what the app already had. Do not discover tools by running them and reading command-not-found failures: run `volli identify` first. Its `env` block reports the resolved `PATH`, where each measured tool (`git`, `gh`, `node`, `npm`, `pnpm`, `yarn`, `bun`) resolves or that it does not, which of them this project actually implies (`env.requiredTools`: `git` for a repository, and for a JavaScript workspace the package manager its lockfile names plus the runtime that manager needs — `node` for npm/pnpm/yarn, nothing extra for bun, and never `gh`), and whether the workspace's dependencies are installed — in this repo, run `pnpm install` if they are absent. A structured Session does not have to ask about that last one: when its workspace has a package manifest and no installed dependencies, a first-message Turn Reminder says so and names the workspace's own lockfile-derived install command, so the first turn can simply run it. A tool that reports `-` but is not in `env.requiredTools` is one this project never runs, not a fault. `volli doctor` audits the same facts and names remedies. A tool that is installed but missing from `volli identify` is a `PATH` adoption failure, not a missing install: run `volli doctor --fix` first. It re-runs adoption for new Sessions and names what changed; the Session already running keeps its startup environment. If the tool remains absent, report it rather than working around it with absolute paths.
...
The same environment carries this Session's **parallelism budget**: `VOLLI_CONCURRENCY_HINT` is how many jobs this Session may assume it has — the machine's cores divided by the Sessions that were working when this one started. Volli writes it in the spellings common toolchains already read, so `cargo`, `make`, `cmake`, `go`, `pytest -n auto`, gradle and vitest self-limit with nothing asked of you: `CARGO_BUILD_JOBS`, `MAKEFLAGS`, `CMAKE_BUILD_PARALLEL_LEVEL`, `GOFLAGS`, `PYTEST_XDIST_AUTO_NUM_WORKERS`, `UV_THREADPOOL_SIZE`, `GRADLE_OPTS`, `VITEST_MAX_WORKERS`. Pass the hint by hand to anything that reads no environment — `jest --maxWorkers=$VOLLI_CONCURRENCY_HINT`, `xargs -P "$VOLLI_CONCURRENCY_HINT"`. A variable your own login shell exported is never overwritten: an exported `MAKEFLAGS=-j16` stays exactly as you set it. A budget only ever LOWERS a project's own cap, never raises it: in this repository `vitest.workers.ts` resolves `maxWorkers` as `min(2, VITEST_MAX_WORKERS)`, so a hint of 1 gives one worker and a hint of 8 still gives two. An explicit `--maxWorkers` flag outranks both — that is how CI, which has its runner to itself, takes every core (`vp run -r test:coverage --maxWorkers=$(nproc)`), and how you take more for one deliberate invocation.
