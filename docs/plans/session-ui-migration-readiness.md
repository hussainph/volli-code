...
- `recover()` now serves the Session it has: a live attachment reconciles, a Session without one re-attaches on the same id (reopening the stream alongside), and only a Session that never got created starts over. Retry after a refused attach no longer files a new durable Session per press.
...
### Landed on `session/transport-edge` (2026-08-05)

Workstream 1 is done, plus the settings surface that proves it. The Runtime Catalog is routed over IPC with `projectId` as the routing key on `inspect` and `save` (`resolve` stays global — it never reads a directory): a per-directory hub in `main/runtime-catalog-hub.ts` resolves a known project to its path, no id to `$HOME`, and an unknown id to NOT_FOUND rather than a silent fallback. The wire protocol lives in `packages/shared/src/session-rpc-wire.ts` (the renderer cannot type-import `src/main`, and shared cannot import the router without a cycle; main re-asserts the frozen allow-list `satisfies` the router's procedures). The preload door is `api.sessionRpc`, and `renderer/src/lib/session-rpc-ipc-link.ts` is the terminating tRPC link over it — it absorbs the ack/frame race with a bounded unclaimed-frame buffer, cancels a subscription abandoned before its ack, surfaces every tracked event id as the result id, and deliberately has no reconnect loop: the cursor is in the consumer's hands, and the reconnecting client remains the future controller's job. `RuntimeCatalogProvider` is mounted in the app root, so `RuntimeCatalogSettings` renders real providers in the shipped app; the harness settings category is now a multi-harness surface with a validated per-harness binary override (live on the next attach — the adapter grew `invalidateBinary()`). The `opencode serve` child now receives the login-shell PATH through the adapter's `resolveEnv` hook, and `e2e/opencode-env-smoke.mjs` proves the whole chain under a launchd-like bare PATH; `e2e/harness-settings-persistence-smoke.mjs` proves preferences survive a restart. `main/session-rpc-ipc.ts` is under the 100% coverage gate.

One seam this landing creates, named so the per-project pickers session prices it in: **discovery is per-project, persistence stayed global.** `save` writes the models discovered for one directory into the single `volli:runtime-preferences:<adapterId>` row, and `resolve` reads that row for every project. Today's only writer is the global settings page hitting the global fallback instance, so the pair stays coherent — an in-session per-project picker becomes a second writer and must scope the row or the save.

Still gating the file move:

- ~~project-aware Runtime Catalog routing and the production preload/tRPC link~~ — landed above;
...
- resume from a cursor over IPC, which is **deliberately not built and is not a bug to re-file.** The preload link has now landed and it rides every tracked id out as the result id, so the cursor is in the consumer's hands — what is still missing is the *reconnecting client* that hands one back as `input.lastEventId`, and that is the chat controller's job, not the transport's;
- ~~a packaged-launch proof of the `opencode serve` child's environment~~ — landed above: the adapter's `resolveEnv` hook threads the login-shell PATH into the child, proven by `e2e/opencode-env-smoke.mjs` under a launchd-like bare PATH;
...
Three workstreams. **1 and 2 are independent and can run in parallel; 3 gates the visible quality of the result and is independent of both.** Workstreams 1 and 2 stay off the engine, the RPC contract and the schema. Workstream 3 does not: its frame change moves engine, artifact and RPC stream semantics together, and is planned and validated as a cross-layer contract change (correction 3).

**Workstream 1 — the transport edge. Done in full on `session/transport-edge` (2026-08-05; see that section).** ~~Widen `SessionRouterProcedure` so non-`session.*` routers are visible to the coverage assertion (A2)~~, ~~route the Runtime Catalog with a projectId key through a per-directory hub and mount `RuntimeCatalogProvider` in the app root (A1)~~, ~~the preload bridge and the tRPC link over IPC (A3)~~, ~~done and error frames (C2)~~, ~~binary resolution through `loginShellPath()` and the child's environment proven against a bare launchd PATH (A4)~~ — all landed. Resume from a cursor still waits on a reconnecting consumer, which is the controller's, not the transport's.

**Workstream 2 — the controller reshape.** Change the hook to `useSessionController(sessionId: string)`, delete the `started` ref and the `start()` at `:635-640`, and make the boot effect `snapshot` + `subscribe` (B1). Move `session.create` + `adapter.attach` to a store action returning a sessionId — `stores/sessions.ts` `byOwner` already models the tab set and `sessionOwner` already routes id → owner. ~~Make `recover()` re-attach on the same sessionId~~ **done in the lab controller** — as a fresh provider conversation, because a client attach carries `native: null` and `native_resume` is not its to claim (B2; correction 2); what is left of B2 is the store action. Lift `transcript` / `queued` / `selection` into a per-sessionId store slice outside React, copying `terminal/registry.ts:20` (B3). ~~Teach `terminalSessionRecord` to say "not a terminal session"~~ — it now says nothing at all, honestly, so what is left of B4 is the discriminated listing DTO that lets the sidebar show a structured Session rather than omit it. Decompose `chat-session.tsx` and de-lab the controller (D1, D2).
