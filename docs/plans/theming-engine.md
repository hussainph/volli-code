...
> **The canvas half pivoted on 2026-07-27.** The band-clamped derived canvas that shipped dormant in #141 is dead; the canvas is now the Arc-style vivid model, built and measured in the UI lab as [#151](https://github.com/hussainph/volli-code/pull/151) (`c6369d6`). [§ The Arc pivot](#the-arc-pivot--the-canvas-the-lab-settled) is the current record for everything canvas-shaped and supersedes the derivation described under § Canvas + shaders and in § Open questions. The four follow-up issues that used to carry this work — **#140** (seed picker), **#143** (vivid colours), **#145** (images), **#146** (shaders) — were **closed on 2026-07-27**; the owner is building this hands-on rather than through specs, and everything they carried that is still true was folded into that section.
...
> **Superseded in part.** The colour half of this section — derived gradients — is dead; read [§ The Arc pivot](#the-arc-pivot--the-canvas-the-lab-settled) for what replaced it. The image and shader halves below stand, and the eviction finding and the guard list are the reason the shader half is still worth reading.
...
**The guard's seam exists now.** `currentGpuPressure()` / `subscribeGpuPressure()` (`renderer/src/terminal/gpu-pressure.ts`, #139 as `ccae0c1`, corrected by #144) answer both questions a guard needs — `{ liveContexts, anyWebgl2, pending }` — and a guard should import **only** that, never the registry. Treat `pending > 0` as not-yet-safe. **Known gap, left in deliberately:** the tracker does not dedupe notifications, so opening a second WebGPU terminal fires listeners with an identical `GpuPressure`; a guard that restarts an animation per notification needs its own equality check.

---

## The Arc pivot — the canvas the lab settled

**2026-07-27.** Everything above about a *derived* canvas is superseded here. This section is the record; there are no open issues carrying it.

### What died, and the lesson worth keeping

#141 shipped the canvas layer **dormant**: the full-window Arc arrangement, the veil token family, the transparency sweep, `ring-offset-transparent`, the scope crossfade, the read-time clamp and a pixel-identity proof for `solid` — with no Background control, because dogfooding killed the colour model. Band-locked to `L ∈ [0.105, 0.170]`, `C ≤ 0.022`, one hue, Ember derives `#160d0a → #0d0705 → #060303`: three shades of near-black, gradient and mesh indistinguishable from solid. A picker whose three options look identical reads as broken, not as subtle.

The band was solved correctly and solved **backwards** — from keeping `--muted-foreground` at Lc 60 where that token sits directly on the canvas in the sidebar. **The generalisable lesson: a constraint solved against text drawn on the canvas will always starve the canvas.** The fix is not a looser clamp, it is letting the *text* move.

### What replaces it

The vivid canvas, built and measured in the UI lab as #151 (`apps/desktop/src/renderer/lab/arc/`, `scratches/canvas.tsx`, the `lab.css` seam) — Arc's Space gradient editor, reproduced closely enough that the numbers transfer. Nothing in the app imports any of it; persistence is `localStorage` under `volli-lab:` keys.

- **Authored, not derived.** 1–3 colour pools the user places on the window, defaulting to seed-derived so the feature works for anyone who never opens the picker. Multi-hue, real chroma — the one-hue rule was right for a near-black ramp and wrong for a visible one.
- **Every tunable in one table**, `ARC_TUNING` in `arc/model.ts`, because a gradient is judged by eye and then adjusted. Nothing in it is derived from anything else. `MAX_STOPS` is **read off** the harmony table rather than stated beside it — the two disagreeing is `#NaNNaNNaN` and a blank screen, not a wrong colour.
- **Harmony sets are rotation-closed** — `[0]`, `[0, 180]`, `[0, 120, 240]` in OKLCH `h` — so promoting any stop to primary is lossless.
- **Modes are a change of colour, not a dimming.** Light pulls the authored L into a pastel band `0.83–0.90`; dark subtracts `0.34` then bands to `0.25–0.44`. Measured: a light band at 0.66 strands *both* inks near Lc 50 (light 56.9, dark 42.1) — 0.83 is where the worst-case ink clears once the base fill counts as a surface.
- **Vibrancy is chroma, capped per mode** — floor `0.14`, gains 1.05 light / 0.62 dark, exponent 0.75 so the travel sits in the useful low half, hard caps 0.16 light / 0.09 dark so a neon seed cannot out-shout the app's own accent.
- **Grain is baked into an SVG noise tile** at a pinned 140px edge and a fixed turbulence seed, so the same grain value always produces the same bytes and the texture reads identically at 1× and 2×. Below 0.01 the layer is not emitted at all.

### Legibility: an ink flip, solved against the worst pool

Two candidate inks — near-white carrying a trace of the primary's hue (`L 0.965 / C 0.012`) and a near-black (`L 0.19 / C 0.02`) — chosen between with `apca-w3` against the **worst surface on screen, including the base fill**, never the average. (Apple Music's adaptive Now Playing is the live counter-example: one pair solved against an averaged gradient, and public complaints about illegible text.) The muted ink slides 0.15 of the way from the chosen ink toward the base fill's L — measured, 0.30 lands at Lc 54.3 and 0.22 at 58.3, so 0.15 is the first value that clears 60 with the mute still a visible tier below full ink.

Arc itself precomputes `--arc-palette-minContrastColor` / `maxContrastColor` and **nobody has ever documented the formula**. That opacity is a liability, not a feature — but *"solve once at pick time, cache with the theme, never per-frame"* is the right shape and is what the lab does.

Measured floors at ship: light body **Lc 90.1**, card muted **60.2**, on-canvas muted **≥ 61.0** at the vibrancy-1 worst case, dark ink **82–94**. Pinned by a 60-canvas sweep (8 hues × 3 vibrancies × both modes) plus 19 model tests.

### The card inherits the canvas

`arc/tokens.ts` — the opaque content card stops being stock dark and becomes a derivative of whatever is on the window, on the owner's two directives: the card must belong to the canvas's family, and the ink flip has to be **app-wide**, since helper text *inside* the card is unreadable on a light surface until the whole token set flips with it.

- **Dark delegates entirely** to the app's own `generateThemeTokens`, seeded from the arc primary. Hand-building a second dark ladder would be inventing a way to be wrong.
- **Light is `LIGHT_LADDER`**, a mirror of `generate.ts` — same shape, same floor list, same `solveLightnessForContrast`, inverted rungs. Applied through `applyThemeTokens` so the veils re-solve. Paper tint is `neutralChroma` × 1–3 by vibrancy; **3 is a measured ceiling**, past which warm hues give chroma back to the gamut map and the tint stops being even across the wheel.
- **Two rung placements are load-bearing and test-pinned**, and must not be "fixed" by eye: light `--sidebar` sits **below** `--rail` (the veil solve `C = (T − B(1−α))/α` at α 0.10 can only exceed its base by ~(255 − B)/10 ≈ 2 bytes up there, and `generateVeilTokens` does not clamp — it would emit an out-of-range `rgb()` and composite wrong in silence), and `--rail` is **darker** than `--background` (on paper, receding means darker; a lighter backdrop makes the card read as a hole).

**This is the light mode #70 deferred.** Not shipped, and not a decision — but the expensive half (a ladder whose every foreground is solved rather than picked) exists and is measured.

### The port, when it happens

The lab is a scratch, not a staging branch. What crosses over:

- **Dedicated `--canvas-ink` / `--canvas-ink-muted` tokens**, adopted by the components that draw on canvas. This replaces `lab.css`'s `html[data-lab-canvas]` flip, which is scoped by hand to `[data-slot="sidebar"]` and `.app-region-drag` subtrees — one token cannot serve a near-white card and a mid-tone gradient at once, which is exactly why the port needs its own token family rather than a wider selector.
- **Veils re-solved against the effective canvas** rather than the token they were solved from.
- **The ticket right rail joins the canvas surfaces** — it sits inside the opaque card today.
- **The chrome band's hardcoded `bg-white/[0.06]` surfaces**, which no token owns.
- **The band-clamp read path in `@volli/shared` is replaced by this model** — `canvasLayerBackground`'s clamp, and `theme/canvas.ts`'s derivation. `withCanvas` / `CANVAS_KINDS` / `canvasOf` / `CANVAS_STOP_COUNT` keep their shape; it is the math underneath that goes.
- **`theme-editor.tsx`'s Background row returns** as the stop editor (a comment marks the spot). `theme-combo-box.tsx`'s `preview` / `buttonPreview` / `searchable` props were reverted to main byte-for-byte when the row was withdrawn; re-add them only if the editor turns out to be a combo box, which on the lab's evidence it should not be.

**The safety property to keep:** `e2e/canvas-shots.mjs` must still report **`patches=40 moved=0`** — `solid` staying pixel-identical to pre-canvas main is what makes all of this low-risk. Note the e2e route: theme files are seeded into `<userData>/volli/themes/` on Ember's own seed so the canvas is the only variable, and the check reaches it through the **seed field**, since `withSeed` re-derives the canvas. One dropped check is not worth restoring — *"an outside click ends a Background preview"* — because the picker is inline, so pointer-leave fires first and the assertion passed without testing anything.

### Banked from the closed issues

**Picking the seed (was #140).** Still unbuilt: the seed is a colour well plus a hex field, and it is the most consequential control in the app. Two third-party pickers were reviewed and rejected; the reasons are the constraints on any replacement. It must work **in OKLCH** (a seed that looks fine in HSV can produce a ladder that fails Lc 60 with no warning; a perceptual picker can simply not offer those seeds); it must be **token-driven** rather than a light-mode component hardcoding `bg-[#E3E3E8]` and `text-black` inside a theming engine; **one hue** (the rejected candidate's companions differed only in hue at full saturation — the sunset-versus-material failure); **no opacity**; **Phosphor, never lucide**. The idea worth keeping is showing the **derived set** rather than one chip — and the lab's pad is now the in-repo prior art for that shape: orbs on a plane, a paginated swatch row, and intensity as a *separate* slider, which is independently what Arc's own picker does. Trello's solver is the best-documented one in existence — compute luminance, nudge L in a loop until the target is met — and worth stealing, driven off APCA rather than WCAG 4.5:1, since Atlassian's own writeup says WCAG passed colours that were "still hard to look at."

**Images (was #145).** Decision #74 in full, as the owner settled it: a **bundled curated set plus a custom user image** under `<userData>`; which images ship is the owner's pick, not the implementer's. The bundled half is nearly free — same `assets/` module-graph pipeline as the grain tile (**not** `public/`, which emits a root-absolute URL that 404s under `base: "./"` once packaged), with `assetsInlineLimit: 0` at `vite.config.ts:149` so asset size never decides whether something goes through the app protocol; `e2e/grain-smoke.mjs` is the template for asserting an asset actually decodes over `volli-app://bundle/`. The custom half is a genuinely new surface: `resolvePackagedRendererAsset()` (`src/main/app-protocol.ts`) is today a read-only, traversal-guarded view of the bundled renderer root only, and the CSP at `renderer/index.html:15` is `img-src 'self' data:` — **no `blob:`**. `ThemeCanvas` has no `image` variant yet; adding `kind: "image"` and its persistence guard is part of it. The scrim stays **fixed, not computed** — per-image luminance analysis driving adaptive text tokens is Trello's machinery and was rejected; curated and custom differ in provenance, not in rendering; and **cards stay opaque, strictly** (the #125 grain finding — per-pixel variance in glyph counters and sidebearings — holds here).

**Shaders (was #146).** Wholly covered by [§ Shader guards](#shader-guards) and [§ The context-eviction finding](#the-context-eviction-finding) above, including the seam and the dedupe gap. Two operational notes: `@paper-design/shaders` is still `0.0.77` and the pin must be exact, and `terminal-smoke.mjs` is the one smoke that launches **without** an isolated `--user-data-dir`, so it aborts at launch (exit 0, zero checks) if a dev app holds the single-instance lock — copy it with the flag added to run it alongside a live app.
...
- **Light mode** — deliberately out of scope (#70), *and the ground moved under that*: `lab/arc/tokens.ts` now carries a working light ladder with every foreground APCA-solved, built because the vivid canvas needs an app-wide ink flip. The remaining design pass is smaller than #70 assumed. See [§ The Arc pivot](#the-arc-pivot--the-canvas-the-lab-settled).
...
- ~~**The band is correct and the feature it produces is not worth shipping, so PR B lands with no Background control.**~~ — **the band itself is now dead too** (2026-07-27); [§ The Arc pivot](#the-arc-pivot--the-canvas-the-lab-settled) records what replaced it, including the part of the bullet below that turned out wrong: the successor does **not** carry legibility with #74's scrim, it carries it with an APCA ink flip solved against the worst pool. Kept unstruck below because the reasoning is the whole reason the successor looks the way it does. **The band is correct and the feature it produces is not worth shipping, so PR B lands with no Background control.** Dogfooded against a real window, `L ∈ [0.105, 0.170]` with `C ≤ 0.022` gives Ember the stops `#160d0a` / `#0d0705` / `#060303` — three shades of near-black, and gradient and mesh are indistinguishable from solid. The band is not the mistake; it is the honest answer to a `--muted-foreground` solved to Lc exactly 60 *on the canvas itself*. The mistake was solving the canvas backwards from that constraint. So the machinery ships (layer, veils, transparency sweep, read-time clamp, storage cap, crossfade) with `solid` as the default, and the picker does not — a control whose three options look identical reads as broken rather than subtle. **What supersedes it:** the canvas goes vivid — 1–3 *authored* colors, multi-hue allowed, real chroma — with #74's scrim carrying legibility instead of the band, plus an Arc-style light/dark foreground flip driven by a luminance calculation over the canvas. That PR replaces `theme/canvas.ts`'s derivation and re-exposes the row in `theme-editor.tsx`; `withCanvas`/`CANVAS_KINDS`/`canvasOf` are kept and kept under test for it. `CANVAS_STOP_COUNT` stays at 3 in the meantime.
...
- ~~**PR 5 — which curated images ship**, and is the backend-exposure groundwork its own PR?~~ — **half answered above**: the backend exposure is its own PR (PR A), and images are the full #74 set. Which images, specifically, is still open — it is the owner's pick, and with #145 closed there is no issue tracking it; the constraints are banked in [§ The Arc pivot](#the-arc-pivot--the-canvas-the-lab-settled).
