...
| Tests/release | **Strong core coverage; incomplete diversity proof** | CI runs protected coverage and built assets; an OpenCode chat smoke exists, but it uses a fake OpenCode binary and is not a two-producer certification suite. [CI gate](/Users/phalasiya/Desktop/code/volli-code/.github/workflows/ci.yml:79) [Chat smoke](/Users/phalasiya/Desktop/code/volli-code/apps/desktop/e2e/session-chat-smoke.mjs:1) | Do not expand harness claims faster than producer-level integration/e2e proof. |
...
| Compatibility/evals | OpenCode adapter tests and chat smoke establish a starting pattern. [Adapter test surface](/Users/phalasiya/Desktop/code/volli-code/packages/opencode-adapter/src/index.test.ts:1) | Per-package compatibility matrix, fixture traces, upgrade canary, kill switch, supply-chain review, and user-visible unsupported state. |
