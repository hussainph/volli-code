...
- `4318` — OTLP over HTTP. **This is the port Volli uses.**
...
What it proves that the unit tests cannot: that a collector **accepts** what
Volli emits. A span the OTLP transformer serializes wrongly, an attribute type a
backend rejects, or a trace id the protocol calls invalid all pass an
`InMemorySpanExporter` and fail here.
...
| `execute_tool <activity kind>` | One executed tool, by Volli's bounded `ActivityKind` |
| `volli.agent.turn` | One finished runtime turn |
| `volli.agent.authority` | A call the Session's authority refused, and the rule that refused it |
...
- **No `OTEL_*` anywhere near a tool.** Volli configures its exporter from the
  Settings row, never from the environment, and never writes `OTEL_*` into
