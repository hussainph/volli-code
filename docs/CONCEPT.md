...
| 34 | **The agent surface (July 2026).** The `volli` CLI exposes the planning layer — full read; full write (create/update/move/comment/label/archive); session lifecycle signals; read-only session observation; notifications — and deliberately *not* the app layer or the destruction layer: no archive deletion, no project add/remove, no settings, no driving another session's terminal. **Moves are moves**: a CLI move carries identical semantics to a human drag, including destination-column automation (agents can boot agents once the loop lands), guarded by visibility (notification + attribution), not caps; restraint is soft-enforced via skill norms. #20's staleness rule generalizes to *deliberate moves* (drag or explicit CLI) beating trailing lifecycle events. Events attribute an **actor** — `user` / `session` / `automation` — recording the door, not the keyboard. Full record + contract: `docs/plans/volli-cli.md`. | The CLI as a full app API ("anything a human can do" — lets agents destroy the audit trail, goal 2); read-only CLI (kills orchestration); label-only agent moves; hard recursion caps (limiting, premature) | The board stays the single coordination medium between agents; visibility + audit beat hard limits for a single-player dogfooding phase; the one destructive act (archive delete, #16) stays human. |
| 35 | **Display IDs become workspace-global handles.** Ticket prefixes are workspace-unique (Linear team-key model), enforced as readable validation at project create/edit — no DB constraint migration; the CLI's never-guess ambiguity errors handle legacy dupes. CLI context resolves flag → env (`VOLLI_SESSION`/`VOLLI_TICKET`/`VOLLI_SOCKET`) → cwd inference → hard error. Display IDs are the only ticket handles in the CLI (both directions); UUIDs never cross the socket. `--project` (non-unique names, path fallback, candidates listed on ambiguity) reaches any project with the full agent surface. | Per-project prefixes + mandatory `--project` everywhere (verbose, ambiguous); UUID handles (unreadable, violates #28's display/record split in the other direction); silent first-match guessing | "A display ID names one ticket, anywhere" is a one-sentence rule agents and humans both keep; cross-project reach is what makes an orchestrator an orchestrator. |
| 36 | **CLI runtime + transport.** `volli` is a shell shim exec'ing the app's own Electron (`ELECTRON_RUN_AS_NODE=1`) against a thin client bundle regenerated into `<userData>/bin` each boot; PATH-injected into every Volli-spawned PTY; consent-gated first-launch symlink to `/usr/local/bin` for outside use. Transport: one-shot NDJSON over `<userData>/volli.sock` (0600); all logic server-side in main behind the same command layer the UI uses. Output: pretty on TTY, plain-stable on non-TTY, `--json` everywhere; stable `error[CODE]` vocabulary; exits 0/1/2/3 with 3 = app unreachable. No implicit app launch — recovery is the explicit, idempotent, toast-announced `volli app launch`. | Standalone compiled binary (~60MB second runtime); requiring system Node; JSON-default output (token-heavy, worse transcripts); auto-launching the app on a dead socket (surprising, racy); MCP (re-affirmed non-goal, #6) | Zero-setup where agents live, one runtime, one choke point for every future consumer; text-first CLIs (git, gh) are proven daily by agents; explicit beats implicit for GUI side effects. |
| 37 | **Skill pack + adapter core.** One hub-and-spoke `volli` skill (SKILL.md router + `cli.md` + `orchestration.md` + user-owned `custom/` that updates never touch — the future automation-authoring surface). Global-only install, consent-gated, detected-harnesses-only: one canonical copy at `~/.agents/skills/volli/` (Codex/Cursor/OpenCode all read it) + a `~/.claude/skills/volli` symlink; condensed fenced sections (`<!-- volli:begin -->`) merged into global instruction files for the skill-less tier, never clobbering user content; refresh is hash-guarded (user-edited managed files warn+diff, never silently overwritten); clean uninstall. Slash entry points per harness mechanism (Codex: skill-only — prompts are deprecated). The composed brief hard-wires skill invocation inside Volli sessions. Architecture: shared adapter core + thin per-harness descriptor modules emitting declarative install plans applied by one executor; `harness-command.ts` folds in. | Worktree/repo materialization of skills (dirties repos or demands per-worktree bookkeeping; global + self-scoping descriptions achieve dormancy); one monolithic skill (context pollution); per-harness independent copies (installer sprawl); bespoke automation-builder UI (skills are the industry-standard authoring surface) | Two install targets cover four harnesses; the skill is the product's teaching surface and — via `custom/` — its future automation surface; per-harness knowledge concentrated in one evolvable file each. |
...
The sketch above grew into the full agent surface — command list, output/socket contract, actor attribution, and the harness skill pack — settled in decisions #34–37 and specified in `docs/plans/volli-cli.md`.
